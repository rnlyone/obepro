                                                  ...
                                                  @foreach ($normalalter as $a => $alt)
                                                      @foreach ($krit as $k => $kr)
                                                          <tr>
                                                              ...
                                                              <td colspan="4">{{$sub->find($nilai->where('id_user', $user->id)->where('id_kriteria', $kr->id)->first()->id_subkriteria ?? "")->nama ?? 'NULL'}} | ({{$nilai->where('id_user', $user->id)->where('id_alternatif', $alt->id)->where('id_kriteria', $kr->id)->first()->nilai ?? 'NULL'}})</td>
                                                              <td>{{$kr->target}}</td>
